...
#define CONFIG_AP_SSID "iptime_nano"
#define CONFIG_AP_SECURITY "wpa2_aes"
#define CONFIG_AP_PASS "skshsmsgkskek"

#define ARTIK_CLOUD_BEARER "af949d70428b408d98987d100baa5ef8"
#define ARTIK_CLOUD_DID "29f04c17949f4fcc977e38c4056dbedc"
...
int artik_demo_run(int argc, char *argv[])
{
	...
	float alpha=0.90;
	int  pre_x,now_x,filtered_x;
	int x=2;
	int val1=0,val2=0;

	int btn=3;
	int pre_btn;
	//,now_btn,filtered_btn;
	//float alpha_btn=0.70;


	int danger_count=0;
	int i;
	field_value[0]=1; field_value[1]=1; field_value[2]=1;

	for(i=0;i<20;++i)
	{
		pre_x=analogRead(x);
		now_x=analogRead(x);
		filtered_x=alpha*pre_x + (1-alpha)*now_x;
		usleep(20);
	}
	...
	while(1){
		...
		if( ( (val1-val2)>=5 ) ||  ( (val2-val1)>=5)  ){
					
			pre_btn=analogRead(btn);
			//printf("%d\n",pre_btn);	
			if(pre_btn<10)
			{
				danger_count=0;	
				for(i=0;i<20;++i){	
					pre_btn=analogRead(btn);
					if(pre_btn<10) 	++danger_count;
					usleep(10);
				}
			

				if(danger_count>17){ printf("danger\n");	 send_data_to_artik(field_value);}
			}			
		}  //end if
